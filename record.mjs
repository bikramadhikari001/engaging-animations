/**
 * record.mjs — Record any scene as 1080×1920 MP4 video with audio
 *
 * Usage:
 ...
 *
 * CI: run under xvfb-run for virtual display.
 *   xvfb-run node record.mjs 7 ping-pong
 */
...
async function main() {
    ...
    console.log(`🚀 Recording scene ${SCENE_INDEX + 1} as "${FILE_NAME}"${IS_CI ? ' (CI mode)' : ''}...`);
    const browser = await puppeteer.launch({
        headless: false, // needs headed for audio capture (use xvfb in CI)
        args: [
    ...
    if (!fs.existsSync(WEBM_FILE) || fs.statSync(WEBM_FILE).size === 0) {
        console.error('❌ WebM not found or empty.');
        process.exit(1);
    }
    ...
    try {
        ...
        console.log(`\n✅ ${MP4_FILE}\n📦 ${size} MB | ${WIDTH}×${HEIGHT} | ~${RECORD_SECONDS}s | 🔊 Audio`);
    } catch (e) {
        console.log(`\n✅ WebM only: ${WEBM_FILE}`);
    }
}
